...
(define-fungible-token Light)

(define-data-var contract-owner principal tx-sender)
(define-map approved-contracts principal bool)

(define-data-var token-name (string-ascii 32) "Light")
(define-data-var token-symbol (string-ascii 10) "LGT")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://emerald-wonderful-sloth-20.mypinata.cloud/ipfs/QmW3QU1m2xhbdKNREqcTKJ6B7rctQko12LmorrbAdELvmU"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? Light amount sender recipient))
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance Light who))
)

(define-read-only (get-total-supply)
	(ok (ft-get-supply Light))
)
...
;; @params recipient
;; @returns (response bool)
(define-public (mint-fixed (amount uint) (recipient principal) (memo (optional (buff 34))))
  (mint (fixed-to-decimals amount) recipient memo)
)
